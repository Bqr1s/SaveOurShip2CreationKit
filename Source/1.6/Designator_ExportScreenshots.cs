...
{	
	...
	public class ScreenshotHelperWorldComp : WorldComponent
	{
		...
		/*public string defsString = @"
			ShipTurret_Laser,ShipTurret_Plasma,ShipTurret_ACI,ShipTurret_Kinetic,ShipPartTurretLarge,ShipTurret_Laser_Large,ShipTurret_Plasma_Large,ShipTurret_ACII,ShipTurret_Kinetic_Large,
			ShipTurret_ACIII,ShipTorpedoOne,ShipTorpedoTwo,ShipTorpedoSix,ShipPartTurretSpinal,ShipSpinalBarrelKinetic,ShipSpinalBarrelLaser,ShipSpinalBarrelPlasma,ShipSpinalBarrelPhysic,
			ShipSpinalBarrelMechanite,ShipSpinalAmplifier,ShipSpinalEmitter,Turret_Autocannon,Turret_Sniper,ShipCombatShieldGeneratorMini,ShipCombatShieldGenerator,ArchotechShieldGenerator,
			Turret_MiniTurret,ShipHeatsink,ShipHeatsinkLarge,ShipHeatManifold,ShipHeatBankLarge,ShipHeatsinkAntiEntropy,ShipPurgePort,ShipPurgePortLarge,ShipInside_SolarGenerator,
			ShipInside_SolarGeneratorMech,ShipInside_SolarGeneratorArchotech,Ship_Reactor_Small,Ship_Reactor,ArchotechAntimatterReactor,ShipCapacitorSmall,ShipCapacitor,Ship_SencorCluster,
			Ship_SencorClusterAdv,Ship_LifeSupport,Ship_LifeSupport_Small,ShipCloakingDevice,ShipSalvageBay,ShipSalvageBayNano,Ship_DroneCore,Ship_Thruster,Ship_Engine_Small,Ship_Engine,
			Ship_Engine_Large,Ship_Engine_Archotech,Ship_Engine_Interplanetary,Ship_Engine_Interplanetary_Large,SoS2_Shuttle_Personal,SoS2_Shuttle,SoS2_Shuttle_Heavy,SoS2_Shuttle_Superheavy
			";*/
		public string defsString = @"
			ShipPartTurretSmall,ShipTurret_Laser,ShipTurret_Plasma,ShipTurret_ACI,ShipTurret_Kinetic,ShipPartTurretLarge,ShipTurret_Laser_Large,ShipTurret_Plasma_Large,ShipTurret_ACII,
			ShipTurret_Kinetic_Large,ShipTurret_ACIII,ShipTorpedoOne,ShipTorpedoTwo,ShipTorpedoSix,ShipPartTurretSpinal,ShipSpinalBarrelKinetic,ShipSpinalBarrelLaser,ShipSpinalBarrelPlasma,
			ShipSpinalBarrelPhysic,ShipSpinalBarrelMechanite,ShipSpinalAmplifier,ShipSpinalEmitter,ShipCombatShieldGeneratorMini,ShipCombatShieldGenerator,ArchotechShieldGenerator,Turret_Autocannon,
			Turret_Sniper,Turret_MiniTurret,ShipHeatsink,ShipHeatsinkLarge,ShipHeatManifold,ShipHeatBankLarge,ShipHeatsinkAntiEntropy,ShipPurgePort,ShipPurgePortLarge,ShipInside_SolarGenerator,
			ShipInside_SolarGeneratorMech,ShipInside_SolarGeneratorArchotech,Ship_Reactor_Small,Ship_Reactor,ArchotechAntimatterReactor,ShipCapacitorSmall,ShipCapacitor,Ship_Thruster,Ship_Engine_Small,
			Ship_Engine,Ship_Engine_Large,Ship_Engine_Archotech,Ship_Engine_Interplanetary,Ship_Engine_Interplanetary_Large,Ship_SencorCluster,Ship_SencorClusterAdv,Ship_LifeSupport,Ship_LifeSupport_Small,
			ShipCloakingDevice,ShipSalvageBay,ShipSalvageBayNano,Ship_DroneCore,SoS2_Shuttle_Personal,SoS2_Shuttle,SoS2_Shuttle_Heavy,SoS2_Shuttle_Superheavy
			";
		...
		{
			// Make CSV header
			csvText = "Def name,Label,CombatPoints,SizeX,SizeZ,startingShip,TradeShip,CargoValue,NeverAttacks,NeverWreck,Mass,ThrustToWeight,PowerCap,HeatCap," +
				"Sink to Coolant ratio,Purge Capacity,Crew,Heavy Marines,Has shield,PD Weapons,Small Weapons, Large Weapons,Capital Weapons,Amplifiers,Torpedo Tubes,Total Vehicles";
		...
		{
			// Fill dictionary of ship parts count by def name first, as it is required for earlier columns like shield yes/no
			SpaceShipCache ship = shipMapComp.ShipsOnMap.Values.First();
			Dictionary<string, int> defsCount = new Dictionary<string, int>();
			// Fill with zeros for easier yes/no checks
			foreach (string defString in defsLit)
			{
				defsCount[defString] = 0;
			}
			foreach (Building b in ship.Buildings.Where(b => defsLit.Contains(b.def.defName)))
			{
				defsCount[b.def.defName] += 1;
			}
			List<Pawn> vehicles = shipMap2.mapPawns.AllPawnsSpawned.Where(pawn => pawn is VehiclePawn veh && veh.CompVehicleLauncher != null && veh.CompVehicleLauncher.SpaceFlight &&
				defsLit.Contains(pawn.def.defName)).ToList();
			foreach (VehiclePawn veh in vehicles)
			{
			...
			AddField(currentShip.startingShip);
			...
			Building_ShipBridge bridge = ship.Bridges.Empty() ? null : ship.Bridges.First();
			if (bridge != null)
			{
			...
			// Sink:Coolant ratio. Small heatsink ans small coolant tanks have value of 1.
			// Other variants have value based on venting property for sinks and capacity property for tanks.
			float antiEntropyValue = (float)DefDatabase<ThingDef>.GetNamed("ShipHeatsinkAntiEntropy").GetCompProperties<CompProps_ShipHeat>().heatVent /
				DefDatabase<ThingDef>.GetNamed("ShipHeatsink").GetCompProperties<CompProps_ShipHeat>().heatVent;
			float largHeatsinkValue = (float)DefDatabase<ThingDef>.GetNamed("ShipHeatsinkLarge").GetCompProperties<CompProps_ShipHeat>().heatVent /
				DefDatabase<ThingDef>.GetNamed("ShipHeatsink").GetCompProperties<CompProps_ShipHeat>().heatVent;
			float totalSinkValue = defsCount["ShipHeatsink"] + defsCount["ShipHeatsinkAntiEntropy"] * antiEntropyValue + defsCount["ShipHeatsinkLarge"] * largHeatsinkValue;

			// Yes, def for small coolant tank is named heat manifold
			float largeTankValue = (float)DefDatabase<ThingDef>.GetNamed("ShipHeatBankLarge").GetCompProperties<CompProps_ShipHeat>().heatCapacity /
				DefDatabase<ThingDef>.GetNamed("ShipHeatManifold").GetCompProperties<CompProps_ShipHeat>().heatCapacity;
			float totalTankValue = defsCount["ShipHeatManifold"] + defsCount["ShipHeatBankLarge"] * largeTankValue;

			string sinkToCoolantRatio = "";
			if (totalSinkValue < Mathf.Epsilon && totalTankValue < Mathf.Epsilon)
			{
				sinkToCoolantRatio = "n/a";
			}
			if (totalSinkValue > Mathf.Epsilon && totalTankValue < Mathf.Epsilon)
			{
				sinkToCoolantRatio = "Only sinks";
			}
			else if (totalSinkValue < Mathf.Epsilon && totalTankValue > Mathf.Epsilon)
			{
				// This actually works because coolant tanks leak heat to room and room loses heat to vacuum through outerdoor(s), but not really expected on "normal" enemy ship designs
				sinkToCoolantRatio = "Only tanks";
			}
			else
			{
				if (totalSinkValue >= totalTankValue)
				{
					sinkToCoolantRatio = (totalSinkValue / totalTankValue).ToString("F2") + " to 1";
				}
				else
				{
					sinkToCoolantRatio = "1 to " + (totalTankValue / totalSinkValue).ToString("F2");
				}
			}
			AddField(sinkToCoolantRatio);

			int purgeCapacity = defsCount["ShipPurgePort"] * 20000 + defsCount["ShipPurgePortLarge"] * 100000;
			AddField(purgeCapacity);

			List<String> crewDefs = new List<String>()
			{
				"SpaceCrew",
				"SpaceCrewMarine",
				"SpaceCrewMarineHeavy",
			};
			int crewCount = ship.Buildings.Where(b => crewDefs.Contains(b.def.defName)).Count();
			AddField(crewCount);
			int heavyMarineCount = ship.Buildings.Where(b => b.def.defName.Equals("SpaceCrewMarineHeavy")).Count();
			AddField(heavyMarineCount);

			// Shield Yes/no;
			bool hasShield = defsCount["ShipCombatShieldGeneratorMini"] > 0 || defsCount["ShipCombatShieldGenerator"] > 0 || defsCount["ArchotechShieldGenerator"] > 0;
			AddField(hasShield ? "Yes" : "No");
			...
			AddField(vehicles.Count());

			foreach (string defString in defsLit)
			{
				csvText += "," + defsCount[defString];
			}
	...
	class Designator_ExportScreenshots : Designator
	{
		...
		public override void DesignateSingleCell(IntVec3 loc)
		{
			...
			List<string> excludeDefNames = new List<string>()
			{
				"MechanoidMoonBase",
				"StarshipBowDungeon",
			...
			worldComp.screenshotShips = DefDatabase <ShipDef>.AllDefs.Where((ShipDef d) => !excludeDefNames.Contains(d.defName) && !d.startingDungeon).OrderBy(d => d.defName).ToList();
			while(worldComp.screenshotShips.First().defName != "ContainerFurniture")
			{
				worldComp.screenshotShips.RemoveAt(0);
			}
			//worldComp.screenshotShips.RemoveAt(0);
			//ContainerFurniture
			while (worldComp.screenshotShips.Last().defName != "CorvetteArdent")
			{
				worldComp.screenshotShips.RemoveAt(worldComp.screenshotShips.Count -1);
			}
